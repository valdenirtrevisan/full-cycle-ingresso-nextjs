...
export default async function SpotsPage({
  ...
  const uniqueRows = rowLetters.filter((row, index) => {
    return rowLetters.indexOf(row) === index;
  });
    ...
    <main className="mt-10">
      ...
      <div className="mt-6 flex flex-wrap justify-between">
        ...
        <div className="flex w-full max-w-[478px] flex-col gap-y-6 rounded-2xl bg-secondary px-4 py-6">
          ...
          <p>
            Inteira: {formatPrice(event.price)} <br />
            Meia-entrada: {formatPrice(event.price / 2)}
          </p>
